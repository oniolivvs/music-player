...
test("settings navigation includes APIs and Providers before storage", async () => {
  ...
  assert.deepEqual([...nav.matchAll(/data-tab="([^"]+)"/g)].map(match => match[1]), ["interface", "appearance", "providers", "lyrics", "dependencies", "disk", "data", "system"]);
  assert.match(nav, /Customisation[\s\S]*Interface[\s\S]*Appearance[\s\S]*APIs &amp; Providers[\s\S]*Lyrics &amp; Sources[\s\S]*Dependencies[\s\S]*Disk[\s\S]*Backup[\s\S]*System/);
...
test("lyrics and public source recovery are opt-in across Settings and Start Setup", async () => {
  const html = await readFile(new URL("../src/index.html", import.meta.url), "utf8");
  const main = await readFile(new URL("../src/main.js", import.meta.url), "utf8");
  const settings = await readFile(new URL("../src/settings.js", import.meta.url), "utf8");
  assert.match(settings, /autoLyrics:\s*false[\s\S]*lyricsSaveLocal:\s*false[\s\S]*autoAlternativeSources:\s*false/);
  for (const id of ["setAutoLyrics", "setLyricsSave", "setLyricsMode", "setLyricsLanguage", "setAutoAlternatives"]) assert.match(main, new RegExp(`id="${id}"`));
  for (const id of ["suAutoLyrics", "suLyricsSave", "suLyricsMode", "suLyricsLanguage", "suAutoAlt"]) assert.match(html, new RegExp(`id="${id}"`));
  assert.match(main, /invoke\("lyrics_lookup"/);
  assert.match(main, /pickAlternativeSource/);
  assert.match(html, /id="npLyricsBtn"[\s\S]*id="npAltBtn"/);
});
